...
<script>try{if(localStorage.getItem('ep-dark-mode')==='true')document.body.classList.add('ep-dark');}catch(e){}</script>
    ...
    <div class="ep-app">
        ...
        <aside class="ep-sidebar" id="epSidebar" aria-label="Main navigation">
            ...
            <nav class="ep-nav">
                ...
                <a href="{{ route('enrollment.portal.index') }}" class="ep-nav-link {{ request()->routeIs('enrollment.portal.index') ? 'active' : '' }}">
                    <i class="fas fa-house"></i><span class="ep-nav-label">Home</span>
                </a>
                <a href="{{ route('enrollment.portal.create') }}" class="ep-nav-link {{ request()->routeIs('enrollment.portal.create') ? 'active' : '' }}">
                    <i class="fas fa-file-circle-plus"></i><span class="ep-nav-label">Apply Now</span>
                </a>
                <a href="{{ route('enrollment.portal.status') }}" class="ep-nav-link {{ request()->routeIs('enrollment.portal.status') || request()->routeIs('enrollment.portal.check-status') ? 'active' : '' }}">
                ...
                <a href="{{ route('enrollment.old-student.login') }}" class="ep-nav-link {{ request()->routeIs('enrollment.old-student.*') ? 'active' : '' }}">
                    <i class="fas fa-right-to-bracket"></i><span class="ep-nav-label">Old Student Login</span>
                </a>

                <div class="ep-nav-group-title">Support</div>
                <a href="{{ route('login') }}" class="ep-nav-link">
        ...
        <div class="ep-main">
            <header class="ep-topbar">
                <div class="ep-topbar-left">
                    ...
                    <div>
                        <div class="ep-page-title" style="font-size:1.1rem;margin:0;">@yield('title', 'Enrollment Portal')</div>
                    </div>
                </div>
                <div class="ep-topbar-right d-flex align-items-center gap-2">
                    ...
                    <a href="{{ route('enrollment.portal.create') }}" class="ep-btn ep-btn-primary ep-btn-sm d-none d-md-inline-flex">
    ...
    <a href="{{ route('enrollment.portal.create') }}" class="ep-fab d-md-none" aria-label="Apply now">
