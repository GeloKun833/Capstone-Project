                    ...
                    <div class="row g-3" id="teacherPreviewGrid">
                        @foreach($teachers->take(4) as $teacher)
                            ...
                            <div class="col-6 col-md-3">
                                <button type="button" class="ams-teacher-pick js-open-teacher" data-teacher-id="{{ $teacher->id }}">
                                    ...
                                    <span class="ams-teacher-pick-name">{{ $tName }}</span>
                                </button>
                            </div>
                        @endforeach
                    </div>
                    @if($teachers->count() > 4)
                        <button type="button" class="ams-teacher-pick ams-teacher-pick--more ams-teacher-pick--all mt-3" data-bs-toggle="modal" data-bs-target="#allTeachersModal">
                            ...
                            <span>
                                ...
                                <small class="text-muted">{{ $teachers->count() }} created · +{{ $teachers->count() - 4 }} more</small>
                            </span>
                        </button>
                    @endif
                @endif
                ...
                <div class="row g-3" id="allTeachersGrid">
                    @foreach($teachers as $teacher)
                        @php
                            ...
                            $tPhoto = \App\Support\AvatarUploader::url($teacher->user->avatar ?? $teacher->avatar);
                        @endphp
                        <div class="col-6 col-md-4 col-lg-3 js-teacher-modal-item" data-name="{{ $tName }}">
                            <button type="button" class="ams-teacher-pick js-open-teacher" data-teacher-id="{{ $teacher->id }}" data-from-all="1">
                                <span class="ams-teacher-pick-photo">
                                    <img src="{{ $tPhoto }}" alt="{{ $tName }}" onerror="this.onerror=null;this.src='{{ asset('images/photo_defaults.jpg') }}';">
                                </span>
                                <span class="ams-teacher-pick-name">{{ $tName }}</span>
            ...
            <form method="POST" action="{{ route('class-subject.unified-management') }}" id="teacherGradeForm">
                ...
                <div class="modal-body">
                    <div class="ams-teacher-profile mb-4">
                        <img id="tdPhoto" src="{{ asset('images/photo_defaults.jpg') }}" alt="">
                        <div>
...
(function () {
    ...
    const teachersById = {};
    (@json($teachersJson ?? [])).forEach(function (t) {
        teachersById[String(t.id)] = t;
    });
    const defaultPhoto = @json(asset('images/photo_defaults.jpg'));
    ...
    @if(old('teacher_ids.0'))
        openTeacherDetails(@json((int) old('teacher_ids.0')), false);
    @endif
})();
</script>
@endpush
