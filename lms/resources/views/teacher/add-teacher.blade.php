        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col-12 col-md-8">
                    <h3 class="page-title">Add Details for Teacher</h3>
                    <ul class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('teacher/list/page') }}">Teachers</a></li>
                        <li class="breadcrumb-item active">Add Teacher Details</li>
                    </ul>
                    <div class="alert alert-info mt-3">
                        <i class="fas fa-info-circle me-2"></i>
                        <strong>Note:</strong> This page is for adding professional details to existing teacher users. 
                        To create a new teacher user account, go to <a href="{{ route('list/users') }}" class="alert-link"><strong>User Management → Create User</strong></a> 
                        and select role "Teacher".
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="card">
                    <div class="card-body">
                        <form action="{{ route('teacher/save') }}" method="POST">
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <h5 class="form-title"><span>Professional Details</span></h5>
                                    <p class="text-muted mb-3">
                                        <i class="fas fa-user-tie me-2"></i>Select a teacher user and fill in their professional information. 
                                        <strong>If details already exist, they will be updated (not duplicated).</strong>
                                    </p>
                                </div>
                                <div class="col-12 col-sm-4">
                                    <div class="form-group local-forms">
                                        <label>Select Teacher User<span class="login-danger">*</span></label>
                                        <select class="select select2s-hidden-accessible @error('full_name') is-invalid @enderror" style="width: 100%;" tabindex="-1" aria-hidden="true" id="full_name" name="full_name">
                                            <option selected disabled>-- Select Teacher User --</option>
                                            @foreach($users as $key => $names)
                                                @php
                                                    $hasDetails = \App\Models\Teacher::where('user_id', $names->user_id)->exists();
                                                @endphp
                                                <option value="{{ $names->name }}" data-teacher_id="{{ $names->user_id }}" {{ old('full_name') == $names->name ? "selected" :""}}>
                                                    {{ $names->name }} (ID: {{ $names->user_id }}) {{ $hasDetails ? '✓ Has Details' : '+ Add Details' }}
                                                </option>
                                            @endforeach
                                        </select>
                                        <small class="text-muted">
                                            <i class="fas fa-info-circle"></i> 
                                            ✓ = Already has details (will update), + = New (will create)
                                        </small>
                                        @error('full_name')
                                ...
                                <div class="col-12 col-sm-4">
                                    <div class="form-group local-forms">
                                        ...
                                        <input type="text" class="form-control bg-light" name="user_id" id="teacher_id" placeholder="Auto-filled from selection" value="{{ old('user_id') }}" readonly>
                                        <small class="text-muted">
                                            <i class="fas fa-lock"></i> Auto-filled when you select a teacher
                                        </small>
                                    </div>
                                </div>
                                <div class="col-12 col-sm-4">
