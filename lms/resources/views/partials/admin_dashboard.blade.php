...
<div class="admin-dash">
    ...
    <div class="admin-dash__welcome">
        <div>
            <h1 class="admin-dash__title">Welcome back, {{ $firstName }}</h1>
        ...
        <div class="col-xl-3 col-sm-6">
            <div class="admin-stat-card">
                ...
                <div class="admin-stat-card__body">
                    ...
                    <strong class="admin-stat-card__value">{{ number_format($adminData['totalStudents'] ?? 0) }}</strong>
        ...
        <div class="col-xl-3 col-sm-6">
            <div class="admin-stat-card">
                ...
                <div class="admin-stat-card__body">
                    ...
                    <strong class="admin-stat-card__value">{{ number_format($adminData['totalTeachers'] ?? 0) }}</strong>
        ...
        <div class="col-xl-3 col-sm-6">
            <div class="admin-stat-card">
                ...
                <div class="admin-stat-card__body">
                    ...
                    <strong class="admin-stat-card__value">{{ number_format($adminData['totalSubjects'] ?? 0) }}</strong>
                ...
                <div class="admin-stat-card__body">
                    ...
                    <strong class="admin-stat-card__value">
                        @if($attendanceTotal > 0)
                            {{ $adminData['attendancePercentage'] ?? 0 }}%
                        @else
                            —
                        @endif
                    </strong>
                    <span class="admin-stat-card__hint">
                        @if($attendanceTotal > 0)
                            Based on {{ number_format($attendanceTotal) }} records
                        @else
        ...
        <div class="col-xl-7">
            <div class="admin-panel">
                <div class="admin-panel__header">
                    <div>
                        <h2 class="admin-panel__title">{{ $performance['title'] ?? 'Academic Performance Overview' }}</h2>
        ...
        <div class="col-xl-6">
            <div class="admin-panel admin-panel--fill">
                <div class="admin-panel__header">
                    ...
                    @if(Route::has('activity.log'))
                        <a href="{{ route('activity.log') }}" class="admin-panel__link">View all</a>
                    @endif
                </div>
                <div class="admin-panel__body">
                        ...
                        <ul class="admin-activity-list">
                            @foreach($activities as $activity)
                                ...
                                <li class="admin-activity-list__item">
                                    <span class="admin-activity-list__dot admin-activity-list__dot--{{ $tone }}">
                                        <i class="fas {{ $activity['icon'] ?? 'fa-circle' }}"></i>
                                    </span>
                                    <div class="admin-activity-list__content">
                                        <strong>{{ $activity['title'] ?? 'Activity' }}</strong>
                                        <span>{{ $activity['meta'] ?? '' }}@if($at) · {{ $at->diffForHumans() }}@endif</span>
                                    </div>
                                </li>
                            @endforeach
                        </ul>
                            ...
                            @foreach($events as $event)
                                <li class="admin-event-list__item">
                                    <div class="admin-event-list__date">
                                        <span class="admin-event-list__month">{{ optional($event->start_time)->format('M') }}</span>
                                        <span class="admin-event-list__day">{{ optional($event->start_time)->format('d') }}</span>
                                    </div>
                                    <div class="admin-event-list__content">
                                        <strong>{{ $event->title ?? 'Event' }}</strong>
                                        <span>
                                            {{ optional($event->start_time)->format('g:i A') }}
                                            @if(!empty($event->subject?->subject_name))
                        ...
                        <div class="admin-attendance">
                            <div class="admin-attendance__row">
                                ...
                                <div class="admin-attendance__bar">
                                    <span style="width: {{ min(100, $breakdown['present']) }}%"></span>
                                </div>
                            ...
                            <div class="admin-attendance__row">
                                ...
                                <div class="admin-attendance__bar">
                                    <span class="is-absent" style="width: {{ min(100, $breakdown['absent']) }}%"></span>
                                </div>
                            ...
                            <div class="admin-attendance__row">
                                ...
                                <div class="admin-attendance__bar">
                                    <span class="is-late" style="width: {{ min(100, $breakdown['late']) }}%"></span>
                                </div>
