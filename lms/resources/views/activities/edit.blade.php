    ...
    <div class="content container-fluid dir-page">
        ...
        <div class="page-header">
            <div class="row align-items-start">
                <div class="col">
                    <h3 class="page-title mb-1">Edit Activity</h3>
                    <p class="dir-subtitle">Update this task for {{ $lesson->title }}.</p>
                </div>
                <div class="col-auto text-end">
                    ...
                    <a href="{{ route('lessons.activities.show', [$lesson, $activity]) }}" class="btn btn-outline-secondary dir-btn">Back</a>
                </div>
            </div>
        </div>
        ...
        <form action="{{ route('lessons.activities.update', [$lesson, $activity]) }}" method="POST" id="activityForm">
            @csrf
            @method('PUT')
            <div class="row g-3">
                <div class="col-lg-8">
                        ...
                        <div class="p-3 p-md-4">
                            ...
                            <div class="form-group mb-3">
                                ...
                                <textarea class="form-control @error('instructions') is-invalid @enderror" id="instructions" name="instructions"
                                          rows="6" required>{{ old('instructions', $activity->instructions) }}</textarea>
                            ...
                            <div class="form-group mb-3">
                                ...
                                <input type="date" class="form-control @error('due_date') is-invalid @enderror" id="due_date" name="due_date"
                                       value="{{ old('due_date', optional($activity->due_date)->format('Y-m-d')) }}" required>
                                @error('due_date')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>

                            <label class="act-toggle {{ old('allows_submission', $activity->allows_submission) ? 'is-on' : '' }}" id="submissionToggle">
                                <input class="form-check-input" type="checkbox" id="allows_submission" name="allows_submission" value="1"
                                       {{ old('allows_submission', $activity->allows_submission) ? 'checked' : '' }}>
                                <span>
                    ...
                    <div class="d-flex gap-2 mt-3">
                        ...
                        <a href="{{ route('lessons.activities.show', [$lesson, $activity]) }}" class="btn btn-outline-secondary dir-btn">Cancel</a>
                    </div>
                </div>
