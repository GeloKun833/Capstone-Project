...
$(document).ready(function(){
    // Skip default charts on LMS dashboard — custom charts load from dashboard.blade.php
    var path = (window.location.pathname || '').replace(/\/+$/, '') || '/';
    var isDashboard = path === '/dashboard' || path === '/home';
    
    // Students Performance Chart - Empty (Skip on dashboard)
    if($('#apexcharts-area').length>0 && !isDashboard){
        var options = {
            chart: {
                height: 350,
                type: "line",
    ...
    // Number of Students Chart - Empty (Skip on dashboard)
    if($('#bar').length>0 && !isDashboard){
        var optionsBar = {
            chart: {
                type: 'bar',
                height: 350,
    ...
    // Other charts remain unchanged for compatibility
    if($('#s-line').length>0){
        var sline = {
            chart: {
                height: 350,
